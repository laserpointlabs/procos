...
const StakeholderEntityNode: React.FC<NodeProps<OntologyNodeData>> = ({ data, selected }) => {
    ...
    const iconContainerStyle = {
        ...
        height: '20px',
        marginRight: dadmsTheme.spacing.xs,
        background: `linear-gradient(135deg, ${dadmsTheme.colors.accent.success}, ${dadmsTheme.colors.accent.success}dd)`,
                ...
                <div style={{ fontSize: dadmsTheme.typography.fontSize.xs, color: dadmsTheme.colors.text.secondary, marginTop: dadmsTheme.spacing.xs }}>
                    {Object.entries(properties).slice(0, 3).map(([key, value]) => (
                        <div key={key} style={{ marginBottom: '2px' }}>
                            <span style={{ fontWeight: dadmsTheme.typography.fontWeight.medium }}>{key}:</span>{' '}
...
export default StakeholderEntityNode; 